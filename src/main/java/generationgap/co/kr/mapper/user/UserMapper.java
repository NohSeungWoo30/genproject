...
public interface UserMapper {
    ...
    // @Select 어노테이션으로 간단히 구현
    @Select("SELECT user_idx, user_id, provider, user_name, nickname, birth_date, gender, user_ci, " +
            "password_hash, email, phone, profile_name, introduction, signup_date, user_status, " +
            "last_login_at, update_at, ghost " +
            "FROM users " +
            "WHERE user_id = #{userId}")
    UserDTO findByUserIdIncludeDeleted(@Param("userId") String userId);
